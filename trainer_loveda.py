...
class AD_Trainer(nn.Module):
    ...
    def update_centroids_label(self, feature_memory, label_memory, feature, label, label_contrast):
        ...
        for i in range(feature_memory.size(0)):
            ...
            # NEW ADDITION FROM PREVIOUS COMMIT#
            feat_i = F.normalize(feature_i, p=2, dim=1)
            centroids_i = F.normalize(centroids_i.detach(), p=2, dim=1)
            # NEW ADDITION FROM PREVIOUS COMMIT#
    ...
    def gen_update(self, images, images_t, labels, labels_t, i_iter):
        ...
        with torch.no_grad():
            ...
            pred1_source, pred2_source, _ = self.G_source_eval(images)
        ...
        pred1_aug, pred2_aug, feature_aug = self.G(images)
        ...
        #pred_up is basically the probability distribution whereas pred_label_up in the ground truth y, which is one hot encoder and hence the argmax function 
        ...
        if self.lambda_loss_pseudo_label is not None:
            ...
            ###
            loss_seg2 = self.seg_loss(pred2_aug_up, labels)
        ...
        if self.lambda_loss_clustering_label is not None:
            clu_label_mask = torch.ne(clu_label_up, labels)
    ...
    def evaluate_target(self, test_loader, args, per_class = False,verbose=False):
        ...
        for index, batch in (enumerate(test_loader)):
            ...
            with torch.no_grad():
                output = None
                pred_aux, pred_main, _ = self.G(image.cuda())
        ...
        if per_class:
            name_classes = ['Background', 'Building', 'Road', 'Water',  'Barren', 'Forest', 'Agricultural']
